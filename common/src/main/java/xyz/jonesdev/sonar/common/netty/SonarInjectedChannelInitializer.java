...
package xyz.jonesdev.sonar.common.netty;
...
import xyz.jonesdev.sonar.common.InboundHandler;
...
public final class SonarInjectedChannelInitializer extends ChannelInitializer<Channel> {
  ...
  public static void inject(final @NotNull Channel channel, final Consumer<ChannelPipeline> sonarPipelineInjector) {
    ...
    if (channel.isActive() && !FakeChannelUtil.isFakeChannel(channel)) {
      final InboundHandler inboundHandler = new InboundHandler(sonarPipelineInjector);
