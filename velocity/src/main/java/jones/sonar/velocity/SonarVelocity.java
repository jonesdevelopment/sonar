...
import jones.sonar.velocity.fallback.FallbackPlayerCleaner;
...
public enum SonarVelocity implements Sonar, SonarPlugin<SonarVelocityPlugin> {
    ...
    public void enable(final SonarVelocityPlugin plugin) {
        ...
        var fallback = new FallbackListener((VelocityServer) plugin.getServer(), plugin.getLogger());
        ...
        // Apply cleaner to Fallback
        Sonar.get().getFallback().setCleaner(new FallbackPlayerCleaner());
